...
class PersonTracker(object):
	...
	# rospy.Subscriber(image_topic, Image, self.image_callback)
        posearray_topic="/openpose_pose_array"
	rospy.Subscriber(posearray_topic, PoseArray, self.posearray_callback)
        self.bridge = CvBridge()
    def posearray_callback(self,msg):
        #recieve poses array from measurement
        poses_array=msg.poses
        ...
        x_center=poses_array[0].position.x
        y_center=poses_array[0].position.y
        ...
        if(coin >= 1.0-self.noise_probability): 
            x_noise = int(np.random.uniform(-20, 20))
            y_noise = int(np.random.uniform(-20, 20))
...
if __name__ == '__main__':
	...
	# print("Initialize node")
        tracker_manager = PersonTracker(sys.argv[1] if len(sys.argv) >1 else 0.0)
